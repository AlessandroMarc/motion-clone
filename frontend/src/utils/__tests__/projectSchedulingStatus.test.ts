...
describe('checkProjectSchedulingStatus', () => {
  ...
  it('marks a task fully scheduled when events cover its remaining duration', () => {
    const task = makeTask({ id: 't1', planned_duration_minutes: 60, actual_duration_minutes: 0 });
    const start = hoursFromNow(1);
    const end = hoursFromNow(2); // 60-minute event
    const event = makeTaskEvent('t1', start, end);
    const result = checkProjectSchedulingStatus([task], [event as CalendarEventUnion]);
  ...
  it('marks a task not fully scheduled when events cover less than remaining duration', () => {
    const task = makeTask({ id: 't1', planned_duration_minutes: 120, actual_duration_minutes: 0 });
    const start = hoursFromNow(1);
  ...
  it('accounts for actual_duration when computing remaining duration', () => {
    ...
    const task = makeTask({ id: 't1', planned_duration_minutes: 90, actual_duration_minutes: 30 });
    const start = hoursFromNow(1);
  ...
  it('returns allTasksScheduled:true when all incomplete tasks are fully scheduled', () => {
    const t1 = makeTask({ id: 't1', planned_duration_minutes: 60, actual_duration_minutes: 0 });
    const t2 = makeTask({ id: 't2', planned_duration_minutes: 30, actual_duration_minutes: 0 });
    const e1 = makeTaskEvent('t1', hoursFromNow(1), hoursFromNow(2));
    const e2 = makeTaskEvent('t2', hoursFromNow(3), new Date(hoursFromNow(3).getTime() + 30 * 60000));
    const result = checkProjectSchedulingStatus(
      [t1, t2],
  ...
  it('returns allTasksScheduled:false when only some tasks are scheduled', () => {
    const t1 = makeTask({ id: 't1', planned_duration_minutes: 60, actual_duration_minutes: 0 });
    const t2 = makeTask({ id: 't2', planned_duration_minutes: 60, actual_duration_minutes: 0 });
    const e1 = makeTaskEvent('t1', hoursFromNow(1), hoursFromNow(2));
    const result = checkProjectSchedulingStatus(
      [t1, t2],
  ...
  it('reports no deadline violations when task has no due_date', () => {
    const task = makeTask({ id: 't1', due_date: null, planned_duration_minutes: 60 });
  ...
  it('detects a deadline violation when an event starts after the due_date end-of-day', () => {
    ...
    const task = makeTask({ id: 't1', due_date: dueDate, planned_duration_minutes: 60 });
