...
jest.unstable_mockModule('../../services/googleCalendarService.js', () => ({
  GoogleCalendarService: jest.fn().mockImplementation(() => mockGoogleCalendarService),
}));
...
describe('GET /api/google-calendar/auth', () => {
  test('redirects to OAuth URL', async () => {
    mockGoogleCalendarService.initiateOAuth.mockReturnValue('https://accounts.google.com/oauth?state=user-1');
    const res = await supertest(app)
      .get('/api/google-calendar/auth?user_id=user-1')
      .redirects(0);
    expect(res.status).toBe(302);
    expect(res.headers.location).toContain('accounts.google.com');
    expect(mockGoogleCalendarService.initiateOAuth).toHaveBeenCalledWith('user-1');
  });
...
describe('GET /api/google-calendar/callback', () => {
  test('redirects to frontend with success on valid callback', async () => {
    mockGoogleCalendarService.handleOAuthCallback.mockResolvedValue({ success: true });
    const res = await supertest(app)
    ...
    expect(mockGoogleCalendarService.handleOAuthCallback).toHaveBeenCalledWith('xyz', 'user-1');
  });
  ...
  test('redirects with error when OAuth error param is present', async () => {
    const res = await supertest(app)
      ...
      .redirects(0);
    expect(res.status).toBe(302);
    expect(res.headers.location).toContain('google_calendar_error=access_denied');
  });
  ...
  test('redirects with error when code or state is missing', async () => {
    const res = await supertest(app)
      ...
      .redirects(0);
    expect(res.status).toBe(302);
    expect(res.headers.location).toContain('google_calendar_error=missing_code_or_state');
  });
  ...
  test('redirects with error when handleOAuthCallback returns failure', async () => {
    ...
    const res = await supertest(app)
      ...
      .redirects(0);
    expect(res.status).toBe(302);
    expect(res.headers.location).toContain('google_calendar_error=token_exchange_failed');
  });
});
...
describe('GET /api/google-calendar/status', () => {
  test('returns connection status', async () => {
    mockGoogleCalendarService.getConnectionStatus.mockResolvedValue({ connected: true });
    const res = await supertest(app)
      .get('/api/google-calendar/status?user_id=user-1');
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.data.connected).toBe(true);
    expect(mockGoogleCalendarService.getConnectionStatus).toHaveBeenCalledWith('user-1');
  });

  test('returns 400 when user_id is missing', async () => {
    const res = await supertest(app).get('/api/google-calendar/status');
    expect(res.status).toBe(400);
    expect(res.body.success).toBe(false);
  });

  test('returns 500 when service throws', async () => {
    mockGoogleCalendarService.getConnectionStatus.mockRejectedValue(new Error('DB error'));
    const res = await supertest(app).get('/api/google-calendar/status?user_id=user-1');
...
describe('POST /api/google-calendar/sync', () => {
  test('syncs events successfully', async () => {
    mockGoogleCalendarService.syncEventsFromGoogle.mockResolvedValue({ success: true, synced: 5, errors: [] });
    const res = await supertest(app)
      .post('/api/google-calendar/sync')
      .send({ user_id: 'user-1' });
    expect(res.status).toBe(200);
    ...
    expect(mockGoogleCalendarService.syncEventsFromGoogle).toHaveBeenCalledWith('user-1');
  });
...
describe('DELETE /api/google-calendar/disconnect', () => {
  test('disconnects Google Calendar', async () => {
    mockGoogleCalendarService.disconnectGoogleCalendar.mockResolvedValue(undefined);
    const res = await supertest(app)
      .delete('/api/google-calendar/disconnect')
      .send({ user_id: 'user-1' });
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(mockGoogleCalendarService.disconnectGoogleCalendar).toHaveBeenCalledWith('user-1');
  });
